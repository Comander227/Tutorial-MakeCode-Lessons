...
Choose the Rename Variable option..
...
Change the comparison diamond so it reads ``||variables:projectile||`` ``||sprites:x||`` is greater than (>) ``||variables:Player2||`` ``||sprites:right||``.
```blocks
